...
function convertAsyncAPIOperation2OpenAPIOperation(
  ...
  const asyncOpMessageOneOf: Referenced<OpenAPISchema>[] = (asyncOpMessage as any).oneOf;
  if (asyncOpMessageOneOf) {
    schema = { oneOf: [] };
    for (const asyncOpMessageOne of asyncOpMessageOneOf) {
      schema.oneOf?.push((parser.deref(asyncOpMessageOne).resolved as any).payload);
    }
  }
